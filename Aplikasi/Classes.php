        ...
        case ccsDate:
          ...
          if(!$DateValidation) {
            if (is_array($Format)) {
              ...
              $this->Errors->addError($this->Caption . " harus diisi. Gunakan format berikut: " . $FormatString . ". (" . htmlspecialchars($ParsingValue) .")");
            } else {
              $this->Errors->addError($this->Caption . " harus diisi. (" . htmlspecialchars($ParsingValue) . ")");
            }
          }
          break;
        case ccsBoolean:
...
class clsControl
{
  ...
  function Validate()
  {
    ...
    if($this->Required && $this->Value === "" && $this->Errors->Count() == 0)
    {
      ...
      $this->Errors->addError($FieldName . " harus diisi.");
    }
        ...
        case ccsDate:
          ...
          if(!$DateValidation && $this->Errors->Count() == 0)
          {
            if (is_array($this->Format)) {
              ...
              $this->Errors->addError($this->Caption . " harus diisi. Gunakan format berikut: " . $FormatString . ".");
            } else {
              $this->Errors->addError($this->Caption . " harus diisi.");
            }
          }
          break;
        case ccsBoolean:
        ...
        case ccsDate:
          ...
          if (!$DateValidation)
          {
            if (is_array($this->DBFormat)) {
              ...
              $this->Errors->addError($this->Name . " harus diisi. Gunakan format berikut: " . $FormatString . ".");
            } else {
              $this->Errors->addError($this->Name . " harus diisi.");
            }
          }
          break;
        case ccsBoolean:
...
//clsFileUpload Class @0-86FB0F0E
class clsFileUpload
{
  var $Name;
  var $Caption;
  ...
  function clsFileUpload($Name, $Caption, $TemporaryFolder, $FileFolder, $AllowedMask, $DisallowedFileMasks, $FileSizeLimit)
  {
    ...
    if( !is_dir($TemporaryFolder) ) {
      $this->Errors->addError("Tidak dapat mengupload file pada " . $FieldName . " - folder upload tidak ada.");
    } else if( !is_writable($TemporaryFolder) ) {
      $this->Errors->addError("Anda tidak berhak mengupload file pada " . $FieldName . " ke folder sementara.");
    } else if( !is_dir($FileFolder) ) {
      $this->Errors->addError("Tidak dapat mengupload file pada " . $FieldName . " - folder upload tidak ada.");
    } else if( !is_writable($FileFolder) ) {
      $this->Errors->addError("Anda tidak berhak mengupload file pada " . $FieldName . ".");
    } 
  ...
  function Validate()
  {
    ...
    {
      $FieldName = $this->Caption;
      $this->Errors->addError("File yang disertakan pada field " . $FieldName . " harus diisi.");
    }
    $this->CCSEventResult = CCGetEvent($this->CCSEvents, "OnValidate");
    return ($this->Errors->Count() == 0);
  }
  ...
  function Upload($RowNumber = "")
  {
    ...
    if (strlen(CCGetParam($DeleteControl))) { 
      ...
      if ($this->AllowedMask != "") {
        $GoodFileMask = 0;
        $FileMasks=split(';', $this->AllowedMask);
      ...
      if($HTTP_POST_FILES[$FileControl]["size"] > $this->FileSizeLimit) {
        $this->Errors->addError("Ukuran file pada field " . $FieldName . " terlalu besar.");
      } else if (!$GoodFileMask) {
        $this->Errors->addError("Tipe file pada field " . $FieldName . " tidak diperbolehkan.");
      } else {
