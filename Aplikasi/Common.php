...
//Initialize Common Variables @0-3873A3F3
session_start();
header('Pragma: ');
...
define("TemplatePath", "./");
define("ServerURL", "http://localhost/LCMS/");
define("SecureURL", "");
...
//Connection1 Connection Class @-71ACC591
class clsDBConnection1 extends DB_MySQL
{
        ...
        case ccsDate:
          ...
          if(!$DateValidation && $ErrorClass->Count() == 0) {
            if (is_array($Format)) {
              ...
              $ErrorClass->addError($FieldName . " harus diisi. Gunakan format berikut: " . $FormatString . ".");
            } else {
              $ErrorClass->addError($FieldName . " harus diisi.");
            }
          }
          break;
        case ccsBoolean:
...
//CCLoginUser @0-1A01EDE5
function CCLoginUser($login, $password)
{
    $db = new clsDBConnection1();
    $SQL = "SELECT UserUsername FROM users WHERE UserUsername=" . $db->ToSQL($login, ccsText) . " AND UserPassword=" . $db->ToSQL($password, ccsText);
    $db->query($SQL);
